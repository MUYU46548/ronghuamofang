...
> 半自动长篇小说生成系统 —— 机器包办苦力（素材归并→大纲→写作→检查→润色→Word），你保留审批权和最终精修权。
> 八阶段流水线自动执行，关键节点设审批门：随时暂停、打回、重跑，成本逐笔记账并可熔断。
...
- **八阶段流水线**：素材归并 → 整体大纲 → 逐章大纲 → 逐章写作 → 逻辑检查 → 润色 → Word 成品 → Markdown 分卷导出
- **一键工作流**：步骤条 + 「执行下一步」（`Space`）+ 选阶段费用预估，跑到审批门自动暂停
- **审稿辅助**：章节审查出报告 → 逐条接受/忽略 → 批量精修建议（改稿前自动备份，字数 ±20% 铁律）
- **校对与文风**：确定性校对（标点/错字/节奏，零 token）+ 文风特征分析与偏差报告
- **断点续跑**：每章落盘 + `progress.json` + SQLite 记账，中断/熔断后从断点继续，不重复花钱
- **成本估算透明**：逐次调用记账、按阶段/模型聚合、超预算自动熔断
- **模板与代码分离**：提示词在 `prompts/`、Word 版式在 `templates/`，改文件即可，无需改代码
- **数据本地存储**：素材、大纲、章节、成品全部留在本机；除模型 API 和自动更新（GitHub Releases）外不向外发送数据
...
├── prompts/          # 八阶段提示词模板（可热改）+ reference/ 实战咒语
